...
def main():
    try:
        ...
        # The feedback loop is still technically active via FeedbackHandler, but won't retry.
        logger.info("\n--- Example 2: Simulating Generate and Run Once (max_retries=0) ---")
        # Using a prompt that is likely to fail initially to see if it stops after one attempt
        ...
        results2 = rag_system.generate_code(
            user_prompt=prompt2,
            save_final_script=True,
            enable_feedback_loop=True, # Must be true to use FeedbackHandler logic path
            max_retries=0 
        )
        print_results(results2)

        # Test Case 3: Generation only (no execution, no feedback beyond initial generation)
        # Achieved by setting enable_feedback_loop=False, which implies max_retries=0 for FeedbackHandler
        # and NetUnicornRAG should handle not saving if save_final_script is False.
        ...
        results3 = rag_system.generate_code(
            user_prompt=prompt3,
            save_final_script=False, # Test not saving the final script
            enable_feedback_loop=False # This will make effective_max_retries = 0
        )
        ...
        # Test Case 4: Prompt for ShellTask (known to be missing from docs, expect failure but see retries)
        logger.info("\n--- Example 4: Prompt for ShellTask (expect failure, observe retries) ---")
        prompt4 = "Create a NetUnicorn script that runs a shell command 'echo Hello from ShellTask'. Use ShellTask."
        logger.info(f"Sending prompt to RAG system: \"{prompt4}\"")
        results4 = rag_system.generate_code(
            user_prompt=prompt4,
            save_final_script=True,
            enable_feedback_loop=True,
            max_retries=2 # Explicitly set a couple of retries to observe behavior
        )
